...
static int findLongOption(const slopt_Option opts[], const char * opt) {
    for(int i = 0; opts[i].shortName; ++i) {
        if(opts[i].longName && strcmp(opt, opts[i].longName) == 0) return i;
    }
            ...
            for(size_t j = 1; argv[i][j] != '\0' && argv[i][j] != '=' && argv[i][j] != ':'; ++j) {
                ...
                if((argv[i][j + 1] == '=' || argv[i][j + 1] == ':')
                   && opts[opt].argument != SLOPT_DISALLOW_ARGUMENT) {
                    ...
                    cb(SLOPT_UNEXPECTED_ARGUMENT, argv[i][j],
                       opts[opt].longName ? opts[opt].longName : "",
                       argv[i] + j + 2, pl);
                    break;
                }
                ...
                if(argv[i][j + 1] == '\0' && opts[opt].argument != SLOPT_DISALLOW_ARGUMENT
                   && i < argc - 1 && isText(argv[i + 1])) {
                    ...
                    cb(argv[i][0], argv[i][j], opts[opt].longName ? opts[opt].longName : "",
                        argv[i + 1], pl
                    );
                    ++i;
                    break;
                ...
                if(opts[opt].argument == SLOPT_REQUIRE_ARGUMENT) {
                    // Argument required, none given, error
                    cb(SLOPT_MISSING_ARGUMENT, argv[i][j],
                        opts[opt].longName ? opts[opt].longName : "", "", pl
                    );
                    continue;
                }

                cb(argv[i][0], argv[i][j], opts[opt].longName ? opts[opt].longName : "", "", pl);
                continue;
            }
        }
    }
